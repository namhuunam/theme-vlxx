...
namespace Ophim\ThemeVlxx\Controllers;
...
class ThemeVlxxController
{
    ...
    {
        $filePath = storage_path('app/search_history.txt');

        // Read existing search history
        $searchHistory = [];
        if (file_exists($filePath)) {
            $searchHistory = file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
        }
        if ($request['search'] || $request['filter']) {
            $searchHistory[] = $request['search'];
            // Limit to the last 10 unique searches
            $searchHistory = array_unique($searchHistory);
            $searchHistory = array_slice($searchHistory, -10);

            // Save back to the file
            file_put_contents($filePath, implode(PHP_EOL, $searchHistory));
            ...
            return view('themes::themevlxx.catalog', [
                ...
                'searchHistory' => $searchHistory // Pass search history here
            ]);
        }
    ...
    public function getMovieOverview(Request $request)
    {
        ...
        // Log search history
        $this->logSearchHistory($movie->title);

        $filePath = storage_path('app/search_history.txt');
        $searchHistory = file_exists($filePath) ? file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
    ...
    protected function logSearchHistory($movieTitle)
    {
        $filePath = storage_path('app/search_history.txt');
        ...
        // Add the movie title to the history
        $searchHistory[] = $movieTitle;
        $searchHistory = array_unique($searchHistory);
        $searchHistory = array_slice($searchHistory, -10); // Keep only the last 10 entries
    ...
    public function getMovieOfCategory(Request $request)
    {
        ...
        $this->logSearchHistory($category->name);
        $filePath = storage_path('app/search_history.txt');
        $searchHistory = file_exists($filePath) ? file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
        return view('themes::themevlxx.catalog', [
            'data' => $movies,
    ...
    public function getMovieOfRegion(Request $request)
    {
        ...
        $this->logSearchHistory($region->name);
        $filePath = storage_path('app/search_history.txt');
        $searchHistory = file_exists($filePath) ? file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
        return view('themes::themevlxx.catalog', [
            'data' => $movies,
    ...
    public function getMovieOfType(Request $request)
{
    ...
    // Log the search history using the catalog name
    $this->logSearchHistory($catalog->name);

    // Read the search history for display
    $filePath = storage_path('app/search_history.txt');
    $searchHistory = file_exists($filePath) ? file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
